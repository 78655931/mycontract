...
var navTab = {
    ...
    _closeOtherTab: function(index) {
        ...
        if (index > 0) {
           // var str$ = ":eq(" + index + ")"this._getTabs().not(str$).filter(":gt(0)").remove();
...
function navTabPageBreak(args, rel) {
    ...
    if (args.targetType) params[params.length] = {
        name: "targetType",
        value: args.targetType
    };
    if (rel) {
        var $box = $("#" + rel);
        $box.ajaxUrl({
    ...
    } else if (form) navTab.reload($(form).attr("action"), {
        data: params,
    ...
    $.fn.extend({
        ...
        itemDetail: function() {
            ...
            function tdHtml(field, itemDetail) {
                var html = '',
                fieldName = itemDetail + '[#index#].' + field.name;
